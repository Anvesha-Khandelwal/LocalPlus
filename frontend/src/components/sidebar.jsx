...
export default function Sidebar() {
  ...
  return (
    <aside style={{ width:240, height:'100vh', position:'fixed', top:0, left:0, background:'#fff', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', padding:'24px 0' }}>
      <div style={{ padding:'0 20px 24px', borderBottom:'1px solid var(--border)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
          ...
          <span style={{ fontFamily:'var(--font-head)', fontWeight:700, fontSize:17 }}>Local<span style={{ color:'var(--green)' }}>Plus</span></span>
        </div>
      ...
      <nav style={{ flex:1, padding:'12px', display:'flex', flexDirection:'column', gap:2 }}>
        {links.map(({ to, icon: Icon, label }) => (
          <NavLink key={to} to={to} end={to==='/'} style={({ isActive }) => ({ display:'flex', alignItems:'center', gap:10, padding:'9px 12px', borderRadius:'var(--radius-sm)', textDecoration:'none', fontSize:14, fontWeight:500, color: isActive ? 'var(--green-dark)' : 'var(--muted)', background: isActive ? 'var(--green-light)' : 'transparent' })}>
            <Icon size={17} />{label}
          </NavLink>
        ))}
      </nav>
      <div style={{ padding:'16px 20px', borderTop:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <button onClick={logout} style={{ background:'none', border:'none', color:'var(--muted)', cursor:'pointer' }}><LogOut size={15} /></button>
      </div>
    </aside>
  )
}
