...
export default function SaleEntry() {
  ...
  function addToCart(product) { setCart(c => { const e = c.find(x => x.product.id === product.id); return e ? c.map(x => x.product.id === product.id ? { ...x, qty:x.qty+1 } : x) : [...c,{product,qty:1}] }) }
  function updateQty(id, delta) { setCart(c => c.map(x => x.product.id===id ? {...x,qty:Math.max(0,x.qty+delta)} : x).filter(x => x.qty>0)) }
  ...
  async function confirmSale() {
    ...
    setSaving(true)
    try {
      await Promise.all(cart.map(({product,qty}) => salesAPI.record({product_id:product.id,quantity:qty})))
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:24, alignItems:'start' }}>
        <div>
          <input className="form-input" style={{ marginBottom:16 }} placeholder="Search products…" value={search} onChange={e => setSearch(e.target.value)} />
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', gap:10 }}>
            {filtered.map(p => { const c=inCart(p.id); return (
              <div key={p.id} className="card" style={{ padding:'14px 16px', cursor:'pointer', border:c?'1.5px solid var(--green)':'1px solid var(--border)' }} onClick={() => addToCart(p)}>
            ...
            {filtered.length===0 && <p style={{ color:'var(--muted)', fontSize:13 }}>No products found.</p>}
          </div>
        </div>
        <div className="card" style={{ position:'sticky', top:24 }}>
          <h2 style={{ fontSize:15, fontWeight:600, marginBottom:16 }}>🛒 Cart</h2>
          {cart.length===0 ? <p style={{ color:'var(--muted)', fontSize:13, textAlign:'center', padding:'24px 0' }}>Tap a product to add</p> : (
            <>
              ...
              <div style={{ borderTop:'1px solid var(--border)', paddingTop:14, marginTop:8 }}>
                ...
                <button className="btn btn-primary" style={{ width:'100%', padding:11, display:'flex', alignItems:'center', justifyContent:'center', gap:7 }} onClick={confirmSale} disabled={saving||done}>
                  {done ? <><CheckCircle size={16} />Recorded!</> : saving ? 'Recording…' : 'Confirm Sale'}
                </button>
              </div>
            </>
          )}
