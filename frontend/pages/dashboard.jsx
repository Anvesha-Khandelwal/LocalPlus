  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        {summary.length === 0
          ? <p style={{ color: 'var(--muted)', fontSize: 13 }}>No sales data yet. Record your first sale to see the chart.</p>
          : (
      ...
      {lowStock > 0 && (
        <div className="alert-banner alert-warn" style={{ marginBottom: 16 }}>
          ⚠️ {lowStock} product{lowStock > 1 ? 's are' : ' is'} running low on stock. Go to Inventory to restock.
        </div>
      )}
      ...
      <div className="card">
        ...
        {sales.length === 0
          ? <p style={{ color: 'var(--muted)', fontSize: 13 }}>No sales recorded yet.</p>
          : (
            <table className="lp-table">
              <thead>
                <tr><th>Product ID</th><th>Qty</th><th>Total</th><th>Time</th></tr>
              </thead>
              <tbody>
                {sales.map(s => (
                  <tr key={s.id}>
                    ...
                    <td style={{ color: 'var(--muted)', fontSize: 13 }}>
                      {new Date(s.sold_at).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                    </td>
                  </tr>
                ))}
              </tbody>
