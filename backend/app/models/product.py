...
ORM models for inventory: Product, Supplier, PurchaseOrder, PurchaseOrderItem.

All rows are scoped to tenant_id — queries MUST always filter by tenant_id.
A product with the same SKU can exist in different tenants; they are independent rows.
"""
import uuid
...
class Supplier(Base):
    """A supplier/vendor that a business purchases stock from."""
    ...
    # Average days from placing order to receiving stock — used by restocking engine
...
class Product(Base):
    """
    A product/SKU in the business's inventory.
    cost_price: what the business paid per unit
    selling_price: what the business charges customers
    reorder_point: AI-recommended minimum stock before reorder alert fires
    """
    ...
    unit: Mapped[str] = mapped_column(String(50), default="piece")  # piece, kg, litre, etc.
    ...
    reorder_point: Mapped[int] = mapped_column(Integer, default=10)  # alert when stock <= this
    max_stock: Mapped[int | None] = mapped_column(Integer)           # prevents overstocking
...
class PurchaseOrder(Base):
    """
    A purchase from a supplier (restocking event).
    Created when the owner buys new stock. Items update product.quantity on receipt.
    """
    ...
    tenant_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True)
    supplier_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("suppliers.id"), nullable=True)
    status: Mapped[str] = mapped_column(String(50), default="received")  # pending|received|cancelled
...
class PurchaseOrderItem(Base):
    """One line item in a PurchaseOrder."""
