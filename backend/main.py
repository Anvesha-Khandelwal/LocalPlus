"""
AI Business Copilot — FastAPI Application Entry Point
backend/main.py
See full annotated version from earlier in this conversation for detailed comments.
Creates the app, registers routers, middleware, exception handlers, health check.
"""
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.warning("Redis unavailable on startup: %s — caching disabled.", exc)
...
def create_app() -> FastAPI:
    app = FastAPI(
        ...
        description="Backend for the AI-powered SaaS platform for small Indian retail businesses.",
        version="1.0.0",
...
def _register_middleware(app: FastAPI) -> None:
    ...
    async def timing_middleware(request: Request, call_next: Callable) -> Response:
        ...
        logger.log(log_level, "%-6s %-40s → %d  [%.0fms] req=%s",
...
def _register_routers(app: FastAPI) -> None:
    ...
    app.include_router(ocr.router, prefix=f"{API_PREFIX}/ocr", tags=["OCR / Invoice Scanning"])
...
def _register_exception_handlers(app: FastAPI) -> None:
    ...
    async def validation_exception_handler(request: Request, exc: RequestValidationError) -> JSONResponse:
        ...
        logger.warning("Validation error on %s %s: %s  req=%s", request.method, request.url.path, errors,
                        getattr(request.state, "request_id", "-"))
        return JSONResponse(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            content={"error": "Request validation failed", "detail": errors,
    ...
    async def unhandled_exception_handler(request: Request, exc: Exception) -> JSONResponse:
        logger.exception("Unhandled exception on %s %s  req=%s", request.method, request.url.path,
                          getattr(request.state, "request_id", "-"))
...
async def health_check(request: Request) -> JSONResponse:
    ...
    try:
        if request.app.state.redis:
            ...
            redis_error = "Redis client not initialised"
    except Exception as exc:
        redis_error = str(exc)
...
async def root() -> JSONResponse:
    return JSONResponse(content={
        "message": "AI Business Copilot API",
        "version": app.version,
